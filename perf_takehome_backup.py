...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        Key insight: Process all 256 elements together per round to share tree value loads.
        """
        UNROLL = 8  # Process 8 vectors per sub-batch
        N_VECTORS = batch_size // VLEN  # 32 vectors total for 256 elements
        ...
        # Allocate vector scratch space for ALL vectors (round-major approach)
        # We need to store idx and hash for all 256 elements across rounds
        v_idx = [self.alloc_scratch(f"v_idx_{j}", VLEN) for j in range(N_VECTORS)]
        v_hash = [self.alloc_scratch(f"v_hash_{j}", VLEN) for j in range(N_VECTORS)]

        # Working scratch for current sub-batch (8 vectors)
        ...
        # Scalar addresses for indirect loads (8 per vector in current sub-batch)
        s_addr = [[self.alloc_scratch(f"s_addr_{j}_{k}") for k in range(VLEN)] for j in range(UNROLL)]

        # Address base registers (one per sub-batch vector)
